...
float PID_Calc(Pid_Typedef *Pid)
{
		...
		Pid->POut = Pid->P * Pid->PreError;
		Pid->IOut = Pid->I * Pid->SumError;
		Pid->DOut = Pid->D * Pid->dError;
...
#define NLp   -(3*pstair)
#define NMp	 -(2*pstair)
#define NSp	 -(0.5*pstair)
#define ZEp	 0
#define PSp	 (0.5*pstair)
#define PMp	 (2*pstair)
#define PLp	 (3*pstair)
...
void fuzzy (Fuzzle_Typedef *pid,float e,float ec)
{
     ...
     if(e>PL||e<NL)
	    ...
	    if( etemp == 0)
				...
				eRighttemp=((e-etemp)/(float)stair);
	...
	eLeftIndex =(short) ((etemp-NL)/(float)stair);       //例如 etemp=2.5，NL=-3，那么得到的序列号为5  【0 1 2 3 4 5 6】
	eRightIndex=(short) (eLeftIndex+1);
	  ...
	  //计算EC隶属度
	    ...
	    if( ectemp == 0)
				...
				ecRighttemp=((ec-ectemp)/(float)stair);
		  ...
		  if( ectemp == 0)
		 	...
		 	ecLeftIndex =(short) ((ectemp-NL)/(float)stair);  
